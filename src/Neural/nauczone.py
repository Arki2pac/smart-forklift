...
if __name__ == "__main__":
    ...
    bpn = BackPropagationNetwork((25,15,1), lFuncs)


    #with open('objs.pickle', 'w') as f: 
    	#pickle.dump(bpn.weights, f)


    with open('/home/infokomes/Desktop/Project/smart-forklift/src/Neural/objs.pickle') as f:
    	bpn.weights = pickle.load(f)

    f = open('/home/infokomes/Desktop/Project/smart-forklift/src/Neural/rodzaj.txt', 'w')

    if sys.argv[1] == '0000000000000000000000000':
        f.write('3')
        print 3

    else:

        lvInput = np.array([map(int,sys.argv[1])])

        #print lvInput

        #print round(bpn.Run(lvInput))
        if int(round(bpn.Run(lvInput))) == 0:
            f.write('2')
            print 2
        elif int(round(bpn.Run(lvInput))) == 1:
            f.write('1')
            print 1
