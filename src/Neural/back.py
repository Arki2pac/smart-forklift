...
if __name__ == "__main__":
    ...
    for i in range(lnMax+1):
        err = bpn.TrainEpoch(trainingset.lvInput, trainingset.lvTarget, momentum = 0.7)
        if i % 5000 == 0 and i > 0:
    ...
    trainingset.lvOutput = bpn.Run(trainingset.lvInput)
    for i in range(trainingset.lvInput.shape[0]):
        print("Input: {0} Output: {1}".format(trainingset.lvInput[i], trainingset.lvOutput[i]))
